...
func (d *repository) DeleteTemplate(ctx context.Context, id uint64) error {
	tpl, err := d.GetTemplate(ctx, id)
	if err != nil {
		return errors.Wrap(err, "[repo.template] delete template get data by id err")
	}
...
func (d *repository) ListTemplate(ctx context.Context) (*gorm.DB, error) {
	var tplList []model.TemplateModel
	tplModel := d.orm.Find(&tplList).Model(&model.TemplateModel{})
	if tplModel.Error != nil {
		return tplModel, errors.Wrap(tplModel.Error, "[repo.template] list template err")
	}
	return tplModel, nil
}

func (d *repository) SearchTemplateByName(ctx context.Context, tplName string) (*gorm.DB, error) {
	tplModel := d.orm.Where("template_name LIKE ?", "%"+tplName+"%").Model(&model.TemplateModel{})
	if tplModel.Error != nil {
		return tplModel, errors.Wrap(tplModel.Error, "[repo.template] search template err")
	}
	return tplModel, nil
}
